...
from recbole_metarec.MetaUtils import Task
...
class MetaDataLoader(AbstractDataLoader):
    ...
    '''
    def __init__(self,config, dataset, sampler, shuffle=False, training=False):
    ...
    def transformToTaskFormat(self, training=False):
        ...
        for uid,v in taskDict.items():
            task=self.generateSingleTaskForTrain(uid,v, taskDict.keys(), training)
            finalTaskDict[uid]=task
        return finalTaskDict

    def generateSingleTaskForTrain(self,uid,v,taskDict,training=False):
        '''
        ...
        if training:
            spt=self.dataset.join_with_same_profile_user(spt, taskDict)
        else:
        ...
        if training:
            qrt=self.dataset.join_with_same_profile_user(qrt, taskDict)
        else:
